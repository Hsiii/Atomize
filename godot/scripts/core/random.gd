...
class Rng:
	...
	func _init(seed: String) -> void:
		state = AtomizeRandom.hash_seed(seed)
		if state == 0:
			state = 1

	func next() -> float:
		state = AtomizeRandom.u32(state + 0x6d2b79f5)
		var output := AtomizeRandom.imul(
			AtomizeRandom.u32(state ^ AtomizeRandom.unsigned_right_shift(state, 15)),
			AtomizeRandom.u32(1 | state)
		)
		output = AtomizeRandom.u32(
			output
			^ AtomizeRandom.u32(
				output
				+ AtomizeRandom.imul(
					AtomizeRandom.u32(output ^ AtomizeRandom.unsigned_right_shift(output, 7)),
					AtomizeRandom.u32(61 | output)
				)
			)
		)

		return (
			float(AtomizeRandom.u32(output ^ AtomizeRandom.unsigned_right_shift(output, 14)))
			/ 4294967296.0
		)

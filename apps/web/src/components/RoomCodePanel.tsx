...
export function RoomCodePanel({ value, editable = false, onChange }: RoomCodePanelProps) {
  ...
  const roomCodePreview = Array.from({ length: 4 }, (_, index) => value[index] ?? "");
  ...
  return (
    <label className="code-panel waiting-code-panel room-code-panel room-code-panel-editable" onClick={focusInput}>
      <p className="label">{uiText.roomCode}</p>
      <div className="room-code-visual" aria-hidden="true">
        {roomCodePreview.map((character, index) => (
          <div
            key={`room-code-character-${index}`}
            className={value[index] ? "room-code-character filled" : "room-code-character"}
          >
            {character}
          </div>
        ))}
